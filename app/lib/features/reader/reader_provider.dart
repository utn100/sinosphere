...
enum AnnotationMode { hanviet, pinyin, both, none }
...
class ReaderNotifier extends Notifier<ReaderState> {
  ...
  Future<void> addStarredToBookmarks() async {
    for (final token in state.tokens) {
      final id = token.charId ?? token.wordId;
      if (id == null) continue;
      if (!state.starred.contains(id)) continue;
      if (token.charId != null) {
        await _db.collectionDao.toggleBookmark(token.charId!);
      }
    }
  ...
  Future<List<Token>> _segment(String text) async {
    ...
    while (i < text.length) {
      ...
      // Try longest compound match (max 8 chars)
      CompoundWord? match;
      int matchLen = 1;
